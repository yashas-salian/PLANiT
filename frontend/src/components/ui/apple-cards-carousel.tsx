...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  useEffect(() => {
    if (carouselRef.current) {
      ...
      // Force all items to be visible initially
      const timer = setTimeout(() => {
        if (carouselRef.current) {
          const event = new UIEvent("scroll", { bubbles: true })
  ...
  return (
    <>
      ...
      >
        ...
        <BlurImage src={card.src} alt={card.title || "Card image"} className="object-cover absolute z-10 inset-0" />
      </motion.button>
    </>
  )
}

export const BlurImage = ({ src, alt, className, fill = false }) => {
