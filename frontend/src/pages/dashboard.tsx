...
import welcome_2 from "../components/images/welcome_2.png"
...
import { Calendar1Icon, Facebook, IndianRupee, Instagram, Landmark, Linkedin, MapPinHouse, Phone, PinIcon, Shapes, Users, X, Youtube } from "lucide-react";
        ...
        <div className="overflow-x-hidden bg-purple-200">
          ...
          <div className="bg-[#755EA5] pb-10 pt-6 relative">
            ...
            <div>
                ...
                <div className="ml-20 grid grid-cols-1 md:grid-cols-3 gap-5 mb-16 mr-4">
                  {upcomingEvents.length === 0 ? <BackgroundLines><div className="text-white md:text-white text-2xl font-semibold w-full ml-125"> No Upcoming Events</div></BackgroundLines> :
                    upcomingEvents.map((event, index) => (
                      <div>
                      <PinIcon fill="red" className="relative top-3 -left-2 rotate-315 text-red-500 "/>
                      <div key={index} className="bg-white border border-purple-300 rounded-lg p-4 shadow-md">
                        ...
                        {event.Category === 'Wedding' ? <img src={wedding_1} className="w-100 h-80"></img> : <div></div>}
                        {event.Category === 'Birthday' ? <img src={birthday_1} className="w-100 h-80"  ></img> : <div></div>}
                        {event.Category === 'Anniversary' ? <img src={anniversary_1} className="w-100 h-80"></img> : <div></div>}
                        {event.Category === 'Corporate events' ? <img src={corporate_1} className="w-100 h-80"></img> : <div></div>}
                        {event.Category === 'Babyshower' ? <img src={babyShower_1} className="w-100 h-80"></img> : <div></div>}
                        {event.Category === 'Open mic night' ? <img src={openMic_1} className="w-100 h-80"></img> : <div></div>}
                        {event.Category === 'Engagement' ? <img src={engagement_1} className="w-100 h-80"></img> : <div></div>}
                        <div className="">
              ...
              {viewMode === 'completed' && (
                <div className="ml-20 grid grid-cols-1 md:grid-cols-3 gap-5 mb-16 mr-4">
                  {completedEvents.length === 0 ? <BackgroundLines><div className="text-white md:text-white text-2xl font-semibold w-full ml-125"> No Upcoming Events</div></BackgroundLines> :
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 gap-7 mb-16 w-full ml-8">
                {filteredMembers.map((member, index) => (
                  ...
                  ${animateCards ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}
                   ${hoverMember === member.id ? "scale-105 shadow-xl" : ""} ${activeTeamMember === member.id ? "ring-4 ring-purple-400" : ""
                      }`}
                    style={{
